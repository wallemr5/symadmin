...
helm-test:
	...
	helm upgrade --kubeconfig ${KUBECONFIG} --kube-context dev-tke-gz-bj5-glb --install sym-ctl --namespace sym-admin --set image.tag=${VERSION},image.worker=true,image.master=true,image.reCreate=true,image.threadiness=1,resources.limits.cpu=1,resources.requests.cpu="500m" ./chart/sym-controller
	...
	helm upgrade --kubeconfig ${KUBECONFIG} --kube-context dev-tke-rz-cd-glb --install sym-ctl --namespace sym-admin --set image.tag=${VERSION},image.worker=true,image.master=false,image.reCreate=true,image.threadiness=1,resources.limits.cpu=1,resources.requests.cpu="500m" ./chart/sym-controller
